...
##### QTL analysis ##########################################################
env_type <- c("combined", "highland" ,"lowland")
radix <- "highland"
...
for(radix in env_type) {

cross <- read.cross("csvs",".","input/B73xPT_genotype.csv",
                    na.strings="NA",
                    phefile = paste0("input/B73xPT_",radix,"_phenotype_", set,".csv"),
                    genotypes=c("A","B"))
...
for (i in seq(2, n_pheno , 2)){
  ...
  possibleError <- tryCatch(
    out_hk <-scanone(cross,pheno.col=i, method="hk"),
    error=function(e) e
  )
  
  if(inherits(possibleError, "error")){
    print(paste(radix,trait,possibleError))
  }
  ...
  perm <- scanone(cross, pheno.col=i, n.perm=1000, method="hk", n.cluster = 4)
...
save(significant,   
     file = paste0("QTL/", radix, "_QTL_scan_", set, ".Rdata"))
save(lod_intervals,
     file = paste0("QTL/", radix, "_QTL_intervals_", set, ".Rdata"))
save(thresh,
     file = paste0("QTL/", radix, "_QTL_threshold_",set, ".Rdata"))

# Plot scans ##################################################################
pdf(file = paste0("QTL/", radix, "_scan_", set, ".pdf"))
...
write.csv(toplot, row.names=FALSE,
          file = paste0("QTL/", radix, "_QTL_intervals_", set, ".csv"))


pdf(file = paste0("QTL/", radix, "_QTL_intervals_", set, ".pdf"),
    height = 3, width = 15)

p <- ggbio::autoplot(gr,  aes(color = lod, fill = lod)) + xlim(0,350e6) +
  ggtitle("PTxB73 significant QTLs (1.5 LOD interval, n = 87)",
          paste0("for flowering time and phospholipids: ", radix, " set ", set)) +
